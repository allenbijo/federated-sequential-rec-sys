...
def data_partition(fname, path=None):
    ...
    if path == None:
        f = open('/home/reuben/reuben_code/Pytorch/LLM-Rec-Sys/FedSasrec/data/amazon/%s.txt' % fname, 'r')
    else:
        f = open(path, 'r')
    for line in f:
...
def evaluate_valid_at_k(model, dataset, args, k=10):
    [train, valid, test, usernum, itemnum] = copy.deepcopy(dataset)
    ...
    return NDCG / valid_user, HT / valid_user, precision, recall, F1